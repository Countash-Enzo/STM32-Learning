...
void PWM_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Prescaler=36-1;//PSC，现在是频率为20khz，要占空比为50%的波形，则CCR为50
	...
	TIM_OC3Init(TIM2,&TIM_OCInitStructure);//初始化输出比较通道2（一个通道对应一个PWM，需要多个的话，同时初始化其它的）
...
{
	TIM_SetCompare3(TIM2,Compare);//设置CCR寄存器的值
}