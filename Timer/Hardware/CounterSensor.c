...
void CountSensor_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO,ENABLE);//开启AFIO时钟
	...
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);//配置优先级分组，第二组对应2个响应与2个抢占（相关函数去misc.h找)
	NVIC_InitTypeDef NVIC_Initstructure;
	NVIC_Initstructure.NVIC_IRQChannel=TIM2_IRQn;//TIM2_IRQn为定时器2在NVIC的通道
	NVIC_Initstructure.NVIC_IRQChannelCmd=ENABLE;
	NVIC_Initstructure.NVIC_IRQChannelPreemptionPriority=2;//配置中断优先级（抢占中断）
